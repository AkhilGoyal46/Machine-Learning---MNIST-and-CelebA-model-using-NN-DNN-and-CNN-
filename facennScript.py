...
def nnObjFunction(params, *args):
    ...
    training_data=np.hstack((training_data,np.ones([np.shape(training_data)[0],1])))
    hidden_nodes = sigmoid(training_data@np.transpose(w1))
    hidden_nodes =np.hstack((hidden_nodes, np.ones([np.shape(hidden_nodes)[0],1])))
    ...
    obj_val += -1*np.sum(np.multiply(label,np.log(output)) + np.multiply(np.subtract(1,label),np.log(np.subtract(1,output))))
    ...
    b= np.transpose(np.multiply( np.multiply(np.subtract(1,hidden_nodes[:,:-1]),hidden_nodes[:,:-1]), (np.subtract(output,label)@w2[:,:-1])))
    
    grad_w1 += (b@training_data)
        
    grad_w2 += np.transpose(np.subtract(output , label)) @ hidden_nodes
...
def nnPredict(w1, w2, data):
    ...
    labels = np.array([])
    # Your code here
    data=np.hstack((data,np.ones([np.shape(data)[0],1])))
